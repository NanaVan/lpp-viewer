...
class Bokeh_show():
    ...
    def _panel_Schottky(self):
        ...
        def update_span(attr, old, new):
            ...
            self.Schottky_spectrum_EC_log.title.text = 'Simulated Spectrum (lifetime > 1 s),    ' + " 1 channel = {:.5f} kHz".format(1.25*float(new)/self.iid.win_len)
            self.Schottky_spectrum_EC_linear.title.text = 'Simulated Spectrum (lifetime > 1 s),    ' + " 1 channel = {:.5f} kHz".format(1.25*float(new)/self.iid.win_len)
            self._update(1, 'ISO', None)
            if self.Schottky_checkbox_ec_on.active:
                self._update(1, 'EC', None)
            if self.Schottky_checkbox_show_one_harmonic.active:
        ...
        def update_win_len(attr, old, new):
            ...
            self.Schottky_spectrum_EC_log.title.text ='Simulated Spectrum (lifetime > 1 s),    ' + " 1 channel = {:.5f} kHz".format(1.25*self.iid.span/float(new))
            self.Schottky_spectrum_EC_linear.title.text = 'Simulated Spectrum (lifetime > 1 s),    ' + " 1 channel = {:.5f} kHz".format(1.25*self.iid.span/float(new))
            self._update(1, 'ISO', None)
            if self.Schottky_checkbox_ec_on.active:
                self._update(1, 'EC', None)
            if self.Schottky_checkbox_show_one_harmonic.active:
    ...
    def _initial_Schottky(self):
        ...
        self.Schottky_spectrum_EC_log = figure(width=1000, height=300, title='Simulated Spectrum (lifetime > 1 s)', tools='pan, crosshair, tap, box_zoom, wheel_zoom, zoom_in, zoom_out, undo, redo, reset, save, hover', x_range=(-self.iid.span/2,self.iid.span/2), y_axis_type='log', output_backend='webgl')
        ...
        self.Schottky_spectrum_EC_log.xaxis.axis_label = "{:} MHz [kHz]".format(self.iid.cen_freq)
        self.Schottky_spectrum_EC_log.title.text = 'Simulated Spectrum (lifetime > 1 s),    ' + " 1 channel = {:.5f} kHz".format(1.25*self.iid.span/self.iid.win_len)
